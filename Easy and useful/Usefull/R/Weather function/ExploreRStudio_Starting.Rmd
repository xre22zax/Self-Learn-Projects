...
temps_f <- c(45,48, 52, 52, 55, 51, 42, 43)
```
...
nice_weather(temps_f)
```
```{r}
is_nice_weather <-  function(temp){
  count <-0
  for(t in temp){
    if(t > 60){
      count <- count + 1
...
possible_destination <- list(
Moscow = c(22, 19, 29, 34, 37, 38, 36, 31),
Cape_Town = c(84, 75, 72, 71, 74, 68, 69, 73),
Wellington = c(72, 64, 68, 72, 70, 72, 71, 70),
Tehran = temps_f)
```
...
ggplot(stack(distination_df), aes(x=ind, y=values)) + geom_boxplot() + labs(x="Cities", y="temp") + ggtitle("City Temprature")
```
